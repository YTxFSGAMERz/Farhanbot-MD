...
exports.buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./FarhanMedia/trash/${filename}.gif`, image)
					 child_process.exec(
								`ffmpeg -i ./FarhanMedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./FarhanMedia/trash/${filename}.mp4`
										) 
  await sleepy(4000)
  
	var buffer5  =  await  fs.readFileSync(`./FarhanMedia/trash/${filename}.mp4`)
	Promise.all([unlink(`./FarhanMedia/video/${filename}.mp4`), unlink(`./FarhanMedia/gif/${filename}.gif`)])
	return buffer5
				   }
