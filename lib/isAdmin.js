...
async function isAdmin(sock, chatId, senderId) {
    try {
        ...
        // Suppress known Baileys errors when group metadata isn't available
        if (!err.message?.includes('attrs') && !err.message?.includes('Connection Closed')) {
            console.error(`❌ Error in isAdmin for ${chatId}:`, err.message || err);
        }
